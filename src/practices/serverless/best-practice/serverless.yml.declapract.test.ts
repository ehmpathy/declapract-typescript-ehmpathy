...
provider:
  ...
  environment:
    ...
    then('it should add accessByStage custom block', async () => {
      ...
      expect(fixed.contents).toContain('dev: prep'); // stage=dev uses ACCESS=prep
...
provider:
  ...
  environment:
    ...
    STAGE: \${self:provider.stage} # deploy specifying which stage we're targeting, to enable targeting the correct config + resources (e.g., hit prep db -vs- prod db)
