...
import { fix } from './best-practice/.github/workflows/.test.yml.declapract';
...
`;
  ...
  given('[case3] context with null declared file contents', () => {
    when('fix is applied', () => {
      then('it THROWS — an absent template is an internal invariant violation, not an empty write', async () => {
        // an absent declaredFileContents means declapract could not read the template beside the
        // declaration — a build wired wrong. the OLD `?? ''` swallowed it and shipped a zero-byte
        // workflow to a consumer (a silent break). the fix must fail loud instead.
        const mockContext = {
          declaredFileContents: null,
          getProjectRootDirectory: () => '/mock/project',
        };

        const error = await getError(fix('any content', mockContext as any));

        expect(error.message).toContain('no declaredFileContents for .test.yml');
  ...
  given('[case4] context with undefined declared file contents', () => {
    when('fix is applied', () => {
      then('it THROWS — same invariant as [case3], for the undefined form', async () => {
        const mockContext = {
