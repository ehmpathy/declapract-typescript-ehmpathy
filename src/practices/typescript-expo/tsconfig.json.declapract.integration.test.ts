...
import { check } from './best-practice/tsconfig.json.declapract';
...
describe('typescript-expo tsconfig.json', () => {
  ...
  it('the emitted template is expo-shaped, not node', () => {
    const template = JSON.parse(
      readFileSync(path.join(__dirname, 'best-practice/tsconfig.json'), 'utf-8'),
    );
  ...
  it('the emitted tsconfig template matches snapshot', () => {
    // snapshot the whole shipped template, so a reshape that keeps the pinned tokens green
    // (a reordered path alias, a rephrased extends) reddens a vibecheck in review
    // (rule.require.contract-snapshot-exhaustiveness)
    const template = readFileSync(
      path.join(__dirname, 'best-practice/tsconfig.json'),
      'utf-8',
    );
    expect(template).toMatchSnapshot('typescript-expo tsconfig.json');
  });
});
