...
1. if latest-pkg-version > latest-tag-version:
   - user has manually bumped the version in package.json
   - use latest-pkg-version as-is (no auto-bump)
   - this enables manual control for major releases, pre-releases, or version jumps

2. otherwise:
   - auto-compute next version from max(latest-pkg-version, latest-tag-version) based on conventional commits
...
given(latest-pkg-version is 2.0.0 and latest-tag-version is v1.5.0)
...
given(a repository with no open release PR)
  when(a non-release commit is merged to main)
    then(a new release PR should be created)
      ...
      with(title "chore(release): v{version} 🎉")
...
given(a release PR to be created or updated)
  when(we generate the description)
    then(the first line should be "🐢 noice! ready to let these changes ride?")
...
given(a release tag has been successfully created)
  when(the release is published)
    then(a comment should be added to the source release PR)
      with(format "🌊 released at {release-url}")
