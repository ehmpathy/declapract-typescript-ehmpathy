...
// fix in the tests below is a PARTIAL mock — the fix reads only `relativeFilePath` to compute
// the moved path, never the rest of FileFixContext. the `as any` asserts that partial to the
// full type. this is the repo-wide test idiom (it appears in ~68 peer .declapract.test.ts
// files, most already on main and untouched by this branch); the removal path is a
// declapract-exported context test-factory, a repo-scoped sweep out of this wish's bound.
